#include "Quantization.hpp"

const Matrix8i JpegQuantization::_Y_quantization_table = (Matrix8i() <<
    16, 11, 10, 16, 24,  40,  51,  61,
    12, 12, 14, 19, 26,  58,  60,  55,
    14, 13, 16, 24, 40,  57,  69,  56,
    14, 17, 22, 29, 51,  87,  80,  62,
    18, 22, 37, 56, 68,  109, 103, 77,
    24, 35, 55, 64, 81,  104, 113, 92,
    49, 64, 78, 87, 103, 121, 120, 101,
    72, 92, 95, 98, 112, 100, 103, 99
).finished();

const Matrix8i JpegQuantization::_UV_quantization_table = (Matrix8i() <<
    17, 18, 24, 47, 99, 99, 99, 99,
    18, 21, 26, 66, 99, 99, 99, 99,
    24, 26, 56, 99, 99, 99, 99, 99,
    47, 66, 99, 99, 99, 99, 99, 99,
    99, 99, 99, 99, 99, 99, 99, 99,
    99, 99, 99, 99, 99, 99, 99, 99,
    99, 99, 99, 99, 99, 99, 99, 99,
    99, 99, 99, 99, 99, 99, 99, 99
).finished();

void JpegQuantization::QuantizeY(Ref<MatrixXi> mat) const {
    const int total_rows = mat.rows();
    const int total_cols = mat.cols();
    for (int col = 0; col < total_cols; col += _block_size) {
        for (int row = 0; row < total_rows; row += _block_size) {
            mat.block<_block_size, _block_size>(row, col) =
                (mat.block<_block_size, _block_size>(row, col).array() + _Y_quantization_table.array() / 2) / _Y_quantization_table.array();
        }
    }
}

void JpegQuantization::QuantizeU(Ref<MatrixXi> mat) const {
    const int total_rows = mat.rows();
    const int total_cols = mat.cols();
    for (int col = 0; col < total_cols; col += _block_size) {
        for (int row = 0; row < total_rows; row += _block_size) {
            mat.block<_block_size, _block_size>(row, col) =
                (mat.block<_block_size, _block_size>(row, col).array() + _UV_quantization_table.array() / 2) / _UV_quantization_table.array();
        }
    }
}

void JpegQuantization::QuantizeV(Ref<MatrixXi> mat) const {
    const int total_rows = mat.rows();
    const int total_cols = mat.cols();
    for (int col = 0; col < total_cols; col += _block_size) {
        for (int row = 0; row < total_rows; row += _block_size) {
            mat.block<_block_size, _block_size>(row, col) =
                (mat.block<_block_size, _block_size>(row, col).array() + _UV_quantization_table.array() / 2) / _UV_quantization_table.array();
        }
    }
}

void JpegQuantization::InverseQuantizeY(Ref<MatrixXi> mat) const {
    const int total_rows = mat.rows();
    const int total_cols = mat.cols();
    for (int col = 0; col < total_cols; col += _block_size) {
        for (int row = 0; row < total_rows; row += _block_size) {
            mat.block<_block_size, _block_size>(row, col).array() *= _Y_quantization_table.array();
        }
    }
}

void JpegQuantization::InverseQuantizeU(Ref<MatrixXi> mat) const {
    const int total_rows = mat.rows();
    const int total_cols = mat.cols();
    for (int col = 0; col < total_cols; col += _block_size) {
        for (int row = 0; row < total_rows; row += _block_size) {
            mat.block<_block_size, _block_size>(row, col).array() *= _UV_quantization_table.array();
        }
    }
}

void JpegQuantization::InverseQuantizeV(Ref<MatrixXi> mat) const {
    const int total_rows = mat.rows();
    const int total_cols = mat.cols();
    for (int col = 0; col < total_cols; col += _block_size) {
        for (int row = 0; row < total_rows; row += _block_size) {
            mat.block<_block_size, _block_size>(row, col).array() *= _UV_quantization_table.array();
        }
    }
}

Quantization* QuantizationFactory::GetQuantization(const std::string &type) {
    if (type == "JPEG") {
        return new JpegQuantization;
    } else {
        throw std::logic_error("Unimplemented quantization");
    }
}